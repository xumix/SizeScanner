...
- Timer-backed custom controls must stop their timer whenever they are detached from the visual tree or their active flag becomes false, and only (re)start it when both attached and active; do not leave a `DispatcherTimer` ticking for a control that is invisible or unmounted. `BusySpinnerControl` gates `DispatcherTimer.Start()`/`Stop()` on `_isAttached && IsActive`, stopping from both `OnDetachedFromVisualTree` and its `IsActive`-change handler (`SizeScanner.Avalonia/Views/BusySpinnerControl.cs`).
...
- **Existing-owner/computed-display pattern for cross-view-model progress:** when one view-model needs to present state that another view-model already owns, keep the owner as the single source of truth and add only a computed, read-only projection on the consumer rather than a second mutable copy. `ChartViewModel` owns `ScopeProgressValue`/`IsScopeProgressIndeterminate`/`IsScopeScanning` for scoped scans; `MainWindowViewModel` does not duplicate them, it exposes `DisplayProgressValue`/`DisplayProgressIsIndeterminate`/`IsBusy` as properties computed from either its own root-scan state or `Chart`'s state, and re-raises them from `Chart.PropertyChanged` in `OnChartPropertyChanged` (`SizeScanner.Avalonia/ViewModels/MainWindowViewModel.cs`, `SizeScanner.Avalonia/ViewModels/ChartViewModel.cs`). `ChartViewModel.IsChartScanning` follows the same pattern one level down, combining `IsRootScanInProgress` (set by `MainWindowViewModel`) with its own `IsScopeScanning` into one derived busy flag consumed by the chart overlay.
