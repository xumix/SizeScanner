...
- Current design: Channels and the root-only worker pool were removed. Each fan-out child is scheduled with `Task.Run` into a bounded in-flight window; a linked `CancellationTokenSource` on `WalkContext` records the first non-cancellation failure (`Fail`), cancels sibling subtrees, and `ObserveAsync` drains every pending task after a failure so none of them ends up as an unobserved exception. The synchronous `Scan` boundary always rethrows that first recorded failure when the caller token is not cancelled, regardless of which secondary exception completes at the root first; caller cancellation takes precedence. `WalkAsync`'s blocking `GetAwaiter().GetResult()` on the caller's thread is the only wait in the walk.
- Test coverage: `ScannerCore.Tests/DirectoryWalkEngineParallelTests.cs` now injects faults and asserts completion under failure — `Concurrent_failures_surface_the_first_captured_failure`, `Single_slot_fan_out_completes_without_deadlock`, `Read_failure_in_one_subtree_aborts_the_scan_with_io_exception`, `Failed_subtree_leaves_no_sibling_reads_active_once_the_scan_returns`, and `Cancellation_during_fan_out_throws_operation_canceled` — with cap/failure/cancellation coverage at explicit fan-out levels 2 and 3.
